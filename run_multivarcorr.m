function run_multivarcorr(data_file)
    %%%%% Main function to run multi-variable correlation analysis
    ...
    dataset = data.values;
    [~, d] = size(dataset);
    [RKtau, RKt_pval, n, deltas, relyRange, symbolMatrix_up, symbolMatrix_down] = multivarcorr(d, dataset);
    ...
    fprintf('\n>> List of coefficients Data trends | δ coefficients | τ coefficients (p-value) \n')
    for i = 1:2^d/2
        fprintf("%s%s | %.4f | %.4f (%.4f)\n", symbolMatrix_up(i,:), symbolMatrix_down(i,:), deltas(i), RKtau(i), RKt_pval(i))
    end
    fprintf('\n')
end